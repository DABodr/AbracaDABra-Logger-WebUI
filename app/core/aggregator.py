...
def aggregate_to_mux_groups(
    ...
    """Aggregate matched data into MuxGroup objects.

    Groups by (bloc, ensemble/label, eid) and collects TII details.
    """
    if matched_df.empty:
        # Fall back to processed_df without TX matching
        return _aggregate_without_tx(processed_df, raw_df, time_col)

    mux_groups: list[MuxGroup] = []

    # Determine label column
    label_col = "Label" if "Label" in matched_df.columns else "ensemble_label"

    # Group by block and ensemble
    grouped = matched_df.groupby(["block", label_col, "eid"], dropna=False)
